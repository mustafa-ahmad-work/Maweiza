...
const RamadanSchedule = () => {
    ...
    return (
        <div className="container mx-auto mb-5 px-2">
            <div className="mt-5">
                <div className="mx-auto p-10 bg-gradient-to-br from-primary-900 to-primary-600 rounded-2xl shadow-lg">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                        ...
                        <div className="bg-white/10 p-4 rounded-lg backdrop-blur-sm">
                            <div className="text-center">
                                ...
                                <div className="text-xl text-primary-200">اليوم من رمضان</div>
                                <div className="mt-2 text-sm text-primary-100">{moment().format('dddd، D MMMM YYYY')}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <table className="w-full whitespace-nowrap">
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                        {prayerTimes.map((day, i) => (
                            <tr key={i}
                                className={`${day.isLast10 ? 'bg-yellow-400 hover:bg-yellow-300 dark:hover:bg-yellow-300 text-white' : 'hover:bg-primary-50 dark:hover:bg-zinc-800/80'}`}>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 mt-8">
                {["السحور", "الإفطار", "ليلة القدر", "العشر الأواخر", "رمضان كريم"].map((title, index) => (
                    <div key={index} className="p-6 shadow-md border border-gray-200 rounded-md bg-white dark:bg-zinc-950 dark:border dark:border-zinc-700">
                        <h4 className="text-xl text-primary-600 text-center mb-4">دعاء {title}</h4>
